(function() {
  ...
  core = {
    ...
    'get_js_value': function(item) {
      if (core.vector_$QMARK_(item)) {
        return core.to_array(item);
      } else if (core.map_$QMARK_(item)) {
        return core.hash_to_js_obj(item);
      } else {
        return item;
      }
    },
    'hash_to_js_obj': function(values) {
      var iter, next, obj;
      obj = {};
      iter = values.keys();
      next = null;
